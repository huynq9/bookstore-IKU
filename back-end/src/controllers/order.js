  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.quantity <= 0) {
        return res
          .status(400)
          .json({ error: "Số lượng đặt hàng không hợp lệ" });
      }
    ...
    if (discountCode) {
      ...
      if (!validDiscountCode || validDiscountCode.role !== "userDiscount") {
        ...
        if (validDiscountCode.type === "value") {
          ...
          itemsPrice -= (
            itemsPrice *
            (validDiscountCode.discount / 100)
          ).toFixed(); // Giảm giá theo phần trăm
        }
      }
    }
    const epsilon = 0.01;
  ...
  try {
    ...
    if (order.status === 0 || order.status === 1) {
      ...
      return res
        ...
        .json({
          error:
            "Cannot change shipping address for orders with status other than 0 or 1",
        });
    }
  } catch (error) {
    console.error(error);
