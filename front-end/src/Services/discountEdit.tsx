...
export const VoucherFormEdit = () => {
  const [selectedType, setSelectedType] = useState("");
  const navigate = useNavigate();
  const [editVoucher] = useUpdateVoucherMutation();
  const { data: categories } = useGetCategoriesQuery();
  const { data: bookData } = useGetProductsQuery();
  const validVoucherTypes = ["percent", "value"];
  const { id } = useParams();
  const { data: voucher } = useGetVoucherByIdQuery(id!);
  ...
  const setFields = () => {
    form.setFieldsValue({
      ...
      categoryIds: voucher?.categoryIds.map((item) => item._id),
      bookIds: voucher?.bookIds.map((item) => item._id),
      validDate: [
        moment(voucher?.validFrom, "YYYY-MM-DD HH:mm"),
        moment(voucher?.validTo, "YYYY-MM-DD HH:mm"),
      ],
    });
  };
  return (
    <Form
      ...
      autoComplete="off"
    >
      <Form.Item label="" hidden name="_id">
        <Input />
      </Form.Item>
      <Form.Item
        label="Title"
